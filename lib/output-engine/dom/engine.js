...
var onOffErrorMessage = 'template .on/off/once(...) shoud only used on containers or context or other object that implement on/off/once interface.';

var engine = {
	//___________________________________ Structure Flow
	each: require('./each').each,
	eachTemplates: function(context, node, args) {
		var templates = args[0],
			handler = args[1];
		templates.forEach(function(templ) {
			(handler ? handler(templ) : templ).call(node, context);
		});
	},
