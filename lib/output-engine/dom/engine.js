...
var engine = {
	...
	switch: function(context, node, args, container) {
		...
		var sw = new Switcher(context, node, container, templates, map['default'], destructOnSwitch);
	...
	with: function(context, node, args, container) {
		var path = args[0],
			...
			data = pathIsString ? context.get(path) : path,
			template = args[1];
		if (data === '')
			data = {};
		var ctx = new Context(data, context, pathIsString ? path : null);
		if (pathIsString) // parent to local bind 
		{
			// ctx.binds = ctx.binds || [];
	...
	contentEditable: function(context, node, args) {
		...
		if (value.__interpolable__) {
			...
			var handler = function(event) {
				...
				freeze = false;
				if (onUpdate)
					onUpdate.call(context, { context: context, path: varPath, node: node, value: val });
			};
			var flagValue = true;
	...
	setClass: function(context, node, args) {
		...
		var flagUpdate = function(value, type, path) {
			...
			try {
				...
				console.error('error while setting node class with : ', classValue);
			}
		};
