 ...
 * Only constructs directly the needed string result.
 *
 * That allows really FAST execution of template server side. 
 * (on my dev machine :  under nodejs, for a full page with 1250 nodes it takes approximatly 0.250 ms to output ;)
 * That IS smart isomorphism : 
 ...
 *
 * This output engine needs stabilised context. (see context stabilisation in docs).
 *
...
var methods = {
	...
	agoraView: function(context, args) {
		var channel = args[0],
			template = args[1];
		throw new Error('agoraView has not been implemented yet for string engine.');
	},
	//______________________________________________
	...
	attr: function(context, descriptor, args) {
		var name = args[0],
			value = args[1];
		descriptor.attributes += ' ' + name + (value ? descriptor.attributes += '="' + (value.__interpolable__ ? value.output(context) : value) + '"' : '');
	},
	...
	container: function(context, descriptor, args) {
		var sod = new SOD();
		args[0].toHTMLString(context, sod);
		descriptor.children += sod.children;
	}
};
