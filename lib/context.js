...
function Context(data, parent, path, env) {
	...
	if (parent) {
		...
		this.env = env ? (env.__yContext__ ? env : new Context(env)) : Context.env;
}
...
Context.prototype = {
	...
	set: function(path, value) {
		...
		if (old !== value) {
			if (old instanceof Date && value instanceof Date && old == value)
				return this;
		...
		if (type !== 'push' && type !== 'removeAt')
			for (var j in space) {
				if (j === '_listeners' || j === '_upstreams')
					continue;
				this.notifyAll(type, path, space[j], (value && typeof value === 'object') ? value[j] : value, index);
			}
		return this;
	},
