...
var utils = require('./utils'),
	env = require('./env'),
	AsyncCollector = require('nomocas-utils/lib/async-collector'),
...
function Context(data, parent, path, env) {
	/** 
	 * Context will inherit of AsyncCollector to manage all internal or external asynchroneous calls.
	 * It allows us to know when a context and its descendants are stabilised(i.e.all data are up to date).
	 **/
	AsyncCollector.call(this);
...
Context.prototype = {
	...
	set: function(path, value, fromParent) {
		// console.log('context.set : ', this.path, path, value, fromParent);
		// .set({ ...:..., ... }) case
		...
		if (path.__yContext__)
			return this;
		// console.log('context.set : really apply : ', this.path, path, value, fromParent);
	...
	displaceItem: function(path /* array path */ , indexes /* { fromIndex, toIndex } */ , fromParent) {
		...
		if (indexes.fromIndex >= arr.length || indexes.toIndex >= arr.length)
			throw new Error('displaceItem failed : indexes out of bound. (' + path + ')(' + indexes.fromIndex + ',' + indexes.toIndex + ')(' + arr.length + ')');
		...
		if (space._listeners)
			for (var i = 0, len = space._listeners.length; i < len; ++i) {
				...
				if (!listener) {
					...
					// (probably some previous listener have removed some listeners through cascade)
	...
	onAgora: function(messageName, handler) {
		var agora = (this.env ||  this).data.agora,
			self = this;
	...
	toAgora: function(name) {
		...
		(this.env || this).data.agora.emit.apply((this.env || this).data.agora, args);
		return this;
	},
	...
	clone: function(cloneEnv) {
		return new Context(utils.copy(this.data), this.parent, this.path, (cloneEnv && this.env) ? this.env.clone() : this.env);
	},
	destroy: function() {
		if (this.binds)
...
utils.shallowMerge(AsyncCollector.prototype, Context.prototype);
