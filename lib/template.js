...
Template.prototype = {
	...
	contentEditable: function(value, flag) {
		var varPath, flag;
		...
		if (typeof value === 'string') {
			...
			if (value.__interpolable__) {
				if (value.dependenciesCount > 1)
					throw new Error("template.contentEditable could only depend to one variable.");
				varPath = value.parts[1].dep[0];
			}
		}
		return this.exec('contentEditable', [varPath, value, flag]);
	},
