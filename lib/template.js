...
Template.prototype = {
	...
	unsubscribe: function(path, handler, upstream) {
		return this.context(function(context) { context.unsubscribe(path, handler, upstream); });
	},
	...
	offAgora: function(name, handler) {
		return this.context(function(context) { context.offAgora(name, handler); });
	},
	toAgora: function(name, msg) {
		return this.context(function(context) { context.toAgora(name, msg); });
	},
	...
	switch: function(xpr, map, destructOnSwitch) {
		for (var i in map)
			if (typeof map[i] === 'string')
				map[i] = y().use(map[i]);
		return this.exec('switch', [interpolable(xpr), map, destructOnSwitch]);
	},
