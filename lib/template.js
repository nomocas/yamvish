...
Template.prototype = {
	...
	toggleInArray: function(path, value) {
		return this.context(function(context) {
			context.toggleInArray(path, value);
		});
	},
	del: function(path) {
		return this.context(function(context) {
	...
	// META-LANGUAGE EACH (to modify chain itself)
	// execute provided function binded on current chain, that will receive item and index as argument.
	_each: function(arr, func, self) {
		if (arr.__yTemplate__) {
			var lexems = arr._lexems;
	...
	use: function(name) {
		...
		var method = (name.forEach ? Template.getAPIMethod(name) : name);
	...
	data: function(name, value) {
		return this.exec('data', [name, typeof value !== 'undefined' ? interpolable(value) : undefined]);
	},
	id: function(value) {
	...
	option: function(value, content) {
		return this.tag('option', y().val(value), content);
	},
	cssLink: function(src) {
		return this.tag('link', y().attr('href', src).attr('rel', 'stylesheet'));
	},
...
// HTML5 tag list
['div', 'span', 'ul', 'li', 'button', 'p', 'form', 'table', 'tr', 'td', 'th', 'section', 'code', 'pre', 'q', 'blockquote', 'nav', 'article', 'header', 'footer', 'aside', 'label', 'strong']
...
[1, 2, 3, 4, 5]
.forEach(function(i) {
	Template.prototype['h' + i] = function() {
		return this.h.apply(this, [i].concat([].slice.call(arguments)));
	};
});
